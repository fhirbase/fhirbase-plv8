...
describe 'CORE: CRUD on table with PostgreSQL reserved name', ->
  before ->
    schema.fhir_create_storage(plv8, resourceType: 'Group')

  beforeEach ->
    plv8.execute("SET plv8.start_proc = 'plv8_init'")
    schema.fhir_truncate_storage(plv8, resourceType: 'Group')

  it 'delete', ()->
    created = crud.fhir_create_resource(plv8, allowId: true, resource: {
      id: 'toBeDeleted', resourceType: 'Group'
    })
    crud.fhir_delete_resource(plv8, {id: created.id, resourceType: 'Group'})
    readDeleted = crud.fhir_read_resource(plv8, {
      id: created.id, resourceType: 'Group'
    })
  ...
  it 'create', ()->
    created = crud.fhir_create_resource(plv8, resource: {
      resourceType: 'Group', name: 'foo bar'
    })
    assert.equal(created.name, 'foo bar')

  it 'read', ()->
    created = crud.fhir_create_resource(plv8, resource: {resourceType: 'Group'})
