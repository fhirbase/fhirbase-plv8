\set ucum `cat src/ucum-essence.xml`
\set ns '{{u,http://unitsofmeasure.org/ucum-essence}}'

DROP table IF EXISTS this.ucum_prefixes;
CREATE TABLE this.ucum_prefixes AS (
SELECT
...
DROP table IF EXISTS this.ucum_units;
CREATE TABLE this.ucum_units AS (
SELECT
  (xpath('./@Code', st, :'ns'))[1]::varchar as code,
