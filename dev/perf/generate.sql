...
WITH inserted as (
  ...
  SELECT
  format('{"resourceType":"Patient", "name": [%s], "birthDate":}', name)::jsonb
  FROM (
    SELECT
    json_build_object(
      'family', ARRAY[x.name],
...
--}}}
--{{{
\set male '{"system": "http://hl7.org/fhir/v3/AdministrativeGender", "code": "M", "display": "Male"}'
\set female '{"system": "http://hl7.org/fhir/v3/AdministrativeGender", "code": "F", "display": "Female"}'
...
FROM (
  ...
  case when x.sex = 'f' then :'female' else :'male' end as gender,
